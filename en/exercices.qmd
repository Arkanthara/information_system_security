...
**Structures:** Group → Ring → Field (increasing invertibility)
...
**A:** $2^1=2$, $2^2=4$, $2^3=8\equiv 1$ → $\text{ord}_7(2)=3$
:::
...
**A:** $3^1=3, 3^2=2, 3^3=6, 3^4=4, 3^5=5, 3^6=1$ → generates all elements → **YES**
:::
...
**A:** $365 \times 2026 = 739490$ possibilities → $H = \log_2(739490) \approx 19.5$ bits
:::

::: {.callout collapse="true"}
### 💡 Improved generator

**Q:** Generator $G$: $P(0)=0.5+\delta$, $P(1)=0.5-\delta$. Create $A$: take 2 bits from $G$, keep 01→0 or 10→1, discard 00 and 11. Advantage?
...
Key = permutation of alphabet. Each letter → fixed letter.
...
⚠️ Identical blocks → encrypted identically (weak security)
...
4. Find $d$: $7d \equiv 1 \mod 160$ → $d=23$
...
3. Extended Euclid for $d = e^{-1} \mod 216$ → $d=137$
...
⚠️ Collision implies second preimage (but not preimage)
...
sequenceDiagram
    ...
    Note over Alice,Bob: Public: p, α
    Alice->>Alice: Choose secret x
    Bob->>Bob: Choose secret y
    Alice->>Bob: αˣ mod p
    Bob->>Alice: αʸ mod p
    Alice->>Alice: K = (αʸ)ˣ mod p
    Bob->>Bob: K = (αˣ)ʸ mod p
    Note over Alice,Bob: Shared key: K = αˣʸ mod p
```
